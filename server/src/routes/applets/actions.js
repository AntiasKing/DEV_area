...
module.exports = {
  ...
  detectTwitterAction: function (webhook, applet, user) {
		if (webhook["favorite_events"] && applet.actionID == 2) {
			this.setServiceAction(applet, user, "I just starred a new tweet");
		}
		// else if (webhook["create_events"])
		// 	console.log("create_event");
  },

	setServiceAction: function(applet, user, message) {
		if (applet.serviceName == "facebook")
			this.facebookAction(applet, user, message);
		else if (applet.serviceName == "twitter")
			this.twitterAction(applet, user, message);
		else if (applet.serviceName == "google")
			this.googleAction(applet, user, message);
		else if (applet.serviceName == "twitch")
			this.twitchAction(applet, user, message);
		else if (applet.serviceName == "spotify")
			this.spotifyAction(applet, user, message);
	},

	facebookAction: function(applet, user, message) {
	...
	twitterAction: function(applet, user, message) {
		if (applet.reactionID == 0)
			console.log(applet, user, message);
		if (applet.reactionID == 1)
			console.log(applet, user, message);
